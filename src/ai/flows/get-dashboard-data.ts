...
import type { DrawTarotCardOutput } from './draw-tarot-card';
import type { GenerateAffirmationOutput } from './generate-affirmation';
import type { AngelNumberOutput } from './get-angel-number-meaning';
import type { DailyReadingOutput } from './get-daily-reading';
import type { GetMoonPhaseOutput } from './get-moon-phase';
...
2.  **A Daily Tarot Card:** Randomly draw one card, determine its orientation (upright/reversed), and provide its name, meaning, a short affirmation, and image keywords.
3.  **A Daily Affirmation:** Generate an affirmation for 'confidence' along with a usage tip.
4.  **A Daily Angel Number:** Provide a common angel number, its general spiritual meaning, and a related affirmation.
5.  **A Moon & Stars Reading:**
    -   Calculate the moon phase for the given date (which is "{{{phaseName}}}").
    -   Provide a hemisphere-specific interpretation, ritual, and affirmation for the moon phase.
